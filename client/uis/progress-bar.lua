...
local function interruptProgress(data)
    if not data.useWhileDead and (IsEntityDead(cache.ped) or playerState.isDead) then return true end
...
if isFivem then
    RegisterKeyMapping('cancelprogress', 'Cancel progress bar', 'keyboard', 'x')
end
